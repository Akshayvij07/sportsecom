...
func (c *CouponDB) UpdateCouponById(ctx context.Context, CouponId int, coupon request.Coupon) (UpdatedCoupon domain.Coupon, err error) {
...
/*func (c *CouponDB) IsApplied(ctx context.Context,Code string,cart domain.Cart)error{
	var coupon domain.Coupon
	//var cart domain.Cart

	findCoupon := `SELECT * FROM coupons WHERE code = $1`
	err :=c.DB.Raw(findCoupon, Code).First(&coupon).Error
	if err!=nil{
		return err
	}
	query:=`SELECT * FROM `
